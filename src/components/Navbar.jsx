...
function Navbar() {
    ...
    const { logout, user, setuser,isLogin } = useContext(UserContext)
        ...
        <nav id='desktop-navbar'>
            <div className='flex flex-row justify-between bg-slate-500 text-white p-3   '>
                <div className="brand font-bold animate-pulse text-lg">
                    <Link href={isLogin?'/':''}>Task Manager</Link>
                </div>
              {isLogin &&  <div className="nav-items">
                ...
                { isLogin &&   <button type='button' className='bg-red-400 mx-2 text-white w-20 rounded-xl hover:bg-red-300' onClick={handlelogOut}>LogOut</button>}
                </div>
            </div>
        </nav>
    )
